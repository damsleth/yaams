...
- untried | apply consolidation_boost to the FUSED cons score, not per-contribution: in _fuse(), accumulate raw RRF for cons then multiply once after the loop (or move the *= into _hydrate_consolidation on components.rrf_score), instead of multiplying every contribution inside the rank loop | — | — | DIAGNOSIS: cons covered by BOTH fts+vec get the boost applied to each contribution, effectively squaring their dual-coverage edge over single-index gold items. `medlemssak` (gold item fts0/vec-None, rrf .032, rank4), `russ` (gold cons vec0/fts-None .034, rank3), `troika medlemssak` all show single-index gold items losing to dual-covered cons. Boost-once removes the unfair compounding without changing the single-cons case.
- untried | single-index penalty / co-coverage credit: in _fuse() after the loop, scale each fused entry's rrf_score by a factor f<1 when only one of (fts_rank, vec_rank) is set (e.g. 0.9) — or symmetrically add a small bonus when both are present | — | — | DIAGNOSIS: the most common bury pattern is a gold item present in exactly one index (fts_rank=0,vec=None → rrf≈.032) losing to a doc present in both (rrf≈.054-.066). RRF already adds the two contributions, but a co-coverage signal is a trust proxy distinct from rank position. Sweep f∈[0.85,0.9,0.95]; guard recall@10 (single-index golds must not drop out of top-10).
