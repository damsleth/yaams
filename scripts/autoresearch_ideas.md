...
- untried | single_index_high_rank_credit (asymmetric, UP only): in _fuse(), when a doc is covered by exactly ONE index but ranks very high there (fts_rank<=2 OR vector_rank<=2), add a small additive RRF credit ~+1/(rrf_k+rank+1)*GAMMA, sweep GAMMA∈[0.15,0.3,0.5] | — | — | DIAGNOSIS: tier2_ledger golds are structurally FTS-only (vec_rank=None) so their RRF caps at one index's contribution; tier2_boost (1.2) multiplies but can't close a 2x raw-rrf gap to a dual-index winner — `deployment` gold (tier2, fts_rank=4, vec=None) #2 behind teams item (fts6+vec10) rrf .0514; `hvem er jeg gift med` gold tier2 rrf .0294 vs cons .0515; `sleep timeline` and `Jan Guttormsen` same shape. This is the INVERSE of the discarded co_coverage_credit (which scaled singletons DOWN by f<1 globally and gave +0.000) — here we credit UP, and ONLY when the single index ranks the doc near the top, so a confident sole-index hit isn't structurally beaten by mediocre dual coverage. Risk: lifting single-index noise; the rank<=2 gate is the guard. Verify regressions==0 and recall@10.
...
- kept | bm25_magnitude_blend: blend normalized bm25 magnitude into FTS RRF contribution in _fuse() with alpha=0.15 | +0.0054 dev (0.5463→0.5517) | — | jun20 round 4. Normalized bm25 magnitude (negated, range-scaled) added as alpha*normed*contribution on top of rank-RRF for FTS lists, so exact-lexical-match confidence gaps are visible to ranker. DO NOT REVISIT (alpha=0.15 applied).
