"""Config-path discovery order per hugr suite plan 05.

YAAMS resolves a config file by walking, in order:

  1. ``$YAAMS_CONFIG`` (explicit override)
  2. ``$XDG_CONFIG_HOME/hugr/yaams/config.yaml`` (suite path - where
     ``hugr init`` writes)
...
when both files exist, because ``hugr init`` is the canonical first-
run flow under the hugr suite.
"""
...
def test_suite_path_resolves_without_yaams_config_env(tmp_path, clean_env):
  xdg = tmp_path / "xdg"
  suite = xdg / "hugr" / "yaams" / "config.yaml"
...
def test_suite_path_wins_when_both_exist(tmp_path, clean_env):
  """Decision: suite path wins. ``hugr init`` owns the canonical config
...
def test_unset_xdg_falls_back_to_dot_config_home(tmp_path, clean_env, monkeypatch):
  ...
  suite_idx = next(
    i for i, p in enumerate(rendered) if p.endswith("/hugr/yaams/config.yaml")
  )
  legacy_idx = next(
    i for i, p in enumerate(rendered)
    if p.endswith("/yaams/config.yaml") and "/hugr/" not in p
  )
...
def test_no_config_anywhere_raises_filenotfound(tmp_path, clean_env):
  ...
  assert "hugr/yaams/config.yaml" in msg
